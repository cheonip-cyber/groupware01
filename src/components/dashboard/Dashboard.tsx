  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        <h2 className="mb-2 text-sm font-bold text-slate-700">오늘의 업무</h2>
        {actions.length > 0 ? (
              ...
              <li key={a.label}>
                <button onClick={() => navigate(a.to)}
                  ...
                  <span className={`h-2 w-2 shrink-0 rounded-full ${
                    a.tone === 'red' ? 'bg-red-500' : a.tone === 'violet' ? 'bg-violet-500'
                    : a.tone === 'amber' ? 'bg-amber-500' : 'bg-blue-500'}`} />
                  <span className="flex-1 text-sm font-medium text-slate-700 group-hover:text-blue-600">{a.label}</span>
                  <span className="text-xs text-slate-300 group-hover:text-blue-500">처리하러 가기 →</span>
                </button>
              </li>
            ))}
          </ul>
        ) : (
          <p className="py-1 text-xs font-medium text-emerald-600">✓ 처리할 긴급 항목이 없습니다 (연체·말일 배치·미수금·미발행 기준)</p>
        )}
      </div>
